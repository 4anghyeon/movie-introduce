    ...
    const cardMoveInterval = () => {
      return setInterval(() => {
        if (++recentlyLastIndex >= resultLength) {
          ...
          moveElementByWidth($recentMovieContainer, `${-(100 * recentlyLastIndex)}vw`);
...
document.querySelectorAll('.right-move-button').forEach(elem => {
  elem.addEventListener('click', e => {
    ...
    moveElementByWidth(parentNode, `${Math.max(-CARD_IMAGE_SIZE * cardLastIndex, -MAX_CARD_CONTAINER_SIZE)}vw`);
